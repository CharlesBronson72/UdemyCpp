...
int main()
{
    ...
    auto elapsed_milliseconds = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count() / 1000.0;
    ...
    elapsed_milliseconds = std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count() / 1000.0;
    std::cout << '\n' << "Time for exercise 2 in ns: " << elapsed_milliseconds;
    const auto is_equal3 = std::equal(result_exercise2.begin(), result_exercise2.end(), target.begin());
